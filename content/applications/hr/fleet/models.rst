...
Odoo's **Fleet** app organizes company vehicles by who made the vehicle (the *manufacturer*), and
the specific type of vehicle (the *model*). For example, a `BMW X2` is manufactured by `BMW` (the
*manufacturer*), and the model is `X2`.

Every vehicle must have both a :ref:`manufacturer <fleet/manufacturers>` and a :ref:`model
<fleet/add-model>` associated with it, to be added to the database.
...
The filter :guilabel:`With Models` is active by default, and displays only manufacturers with models
configured in the database. Remove the default filter to view *all* manufacturers.

The manufacturers appear in an alphabetical list. Each manufacturer's card lists how many specific
:ref:`models <fleet/models>` are configured for each particular manufacturer.
...
blank manufacturers form. Only two pieces of information are needed, the :guilabel:`Name` of the
manufacturer, and the logo. Type the name of the manufacturer in the :guilabel:`Name` field, and
...
Unlike :ref:`manufacturers <fleet/manufacturers>`, no models are preconfigured in the **Fleet** app.
When a new vehicle model joins the fleet, the model (and, if necessary, the manufacturer) **must**
...
- :guilabel:`Manufacturer`: Using the drop-down menu, select the manufacturer. If the manufacturer
  is not configured, type in the manufacturer, and click :guilabel:`Create "(manufacturer)"` or
  :guilabel:`Create and edit..`. When the manufacturer is selected, the logo automatically loads in
  the image box in the upper-right corner, if a logo is configured on the manufacturer form.
- :guilabel:`Vehicle Type`: Using the drop-down menu, select one of two preconfigured vehicle types,
  either :guilabel:`Car` or :guilabel:`Bike`. The vehicle types are hardcoded in Odoo, and are
...
- :guilabel:`Category`: Using the drop-down menu, select a :ref:`category <fleet/categories>` for
  the vehicle. To create a new category, type in the category and then click :guilabel:`Create "(new
  category)"`.
...
- :guilabel:`Horsepower Taxation`: Enter the amount of taxes incurred according to the engine
  ...
  only appears if :guilabel:`Horsepower` is selected for the :guilabel:`Power` field.
- :guilabel:`Tax Deduction`: The percentage that can be deducted from taxes is autopolulated based
  on the localization, and **cannot** be modified. This field only appears for certain
  localizations.
...
quotations for vehicles can be created through Odoo's **Purchase** app.

To add a vendor, click :guilabel:`Add` in the upper-right corner. This opens an :guilabel:`Add:
Vendors` pop-up window, with a list of all the vendors currently in the database. Add a vendor by
ticking the checkbox next to the vendor name, then click :guilabel:`Select`. No limitations exist on
