...
.. js:function:: mountView(params, target = null)
   ...
   This helper is typically used in integration or unit tests to render a view (`form`, `list`, `kanban`, etc.)
...
These utilities help manage the Odoo registry during test execution.
...
.. js:function:: restoreRegistry(registry)

    Restores the given registry to its original state captured before the test.

    This function is used in ``afterEach`` hooks to reset the registry to its pre-test content.

    :param registry: The registry to restore.
